...
<body>
    ...
    <div class="container">
        ...
        <div class="branding">
            <a href="https://miyarahub.com" target="_blank">miyarahub.com</a>
            <p style="margin: 5px 0 0; font-size: 12px; color: #6c757d;">by Meir Miyara</p>
        </div>

        <div class="disclaimer">
            <strong>Disclaimer:</strong> This software is provided free of charge "as is", without warranty of any kind, express or implied. Use at your own risk. This software is not affiliated with, endorsed by, or certified by Microsoft Corporation. The developer assumes no liability for any damages, data loss, or system issues arising from the use of this software. By using this software, you accept full responsibility for its operation on your system.
        </div>
    </div>
</body>
</html>"""
    ...
    def _handle_command(self, command_data):
        ...
        try:
            ...
            # URL shortcuts
                ...
                result = self._launch_shortcut(command)
                if result:
                    return result
    ...
    def _launch_shortcut(self, command):
        """Launch a .lnk shortcut from the shortcuts folder."""
        base_dir = os.path.dirname(os.path.abspath(sys.executable if getattr(sys, 'frozen', False) else __file__))
        shortcuts_dir = os.path.join(base_dir, "shortcuts")
        shortcut_path = os.path.join(shortcuts_dir, f"{command}.lnk")
        if os.path.exists(shortcut_path):
            try:
                os.startfile(shortcut_path)
                logger.info(f"Launched shortcut: {command}")
                return {"status": "success", "message": f"Launched shortcut: {command}"}
            except Exception as e:
                logger.error(f"Failed to launch shortcut '{command}': {e}")
                return {"status": "error", "message": f"Failed to launch shortcut: {str(e)}"}
        return None
